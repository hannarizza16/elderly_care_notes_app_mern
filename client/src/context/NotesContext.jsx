...
export function NotesProvider({ children }) {
    ...
    const fetchAllNotes = useCallback(async () => {
        try {
            ...
            const response = await axios.get(`${BACKEND_URL}/api/notesfeed/allnotes`, {
                headers: {
    ...
    const fetchUserNotes = useCallback(async () => {
        try {
            ...
            const response = await axios.get(`${BACKEND_URL}/api/notesfeed/allusernotes`, {
                headers: {
                    Authorization: `Bearer ${token}`,
                }
            });
...
// add notes to dbase
    const addNotesToDb = async (noteData) => {
        try {
            ...
            const response = await axios.post(`${BACKEND_URL}/api/notesfeed/addnote`, noteData, {
                headers: {
                    Authorization:`Bearer ${token}`,
                }
            })
    ...
    const deleteNotesToDb = async (noteId) => {
        try {
            const token = localStorage.getItem('userToken');
            await axios.delete(`${BACKEND_URL}/api/notesfeed/delete/${noteId}`, {
                headers: {
                    Authorization: `Bearer ${token}`,
                }
            });
    ...
    const updateNoteInDb = async (noteData) => {
        try {
            ...
            const response = await axios.put(
                `${BACKEND_URL}/api/notesfeed/update/${noteData._id}`, 
                noteData,
                {
                    headers: {
                        Authorization: `Bearer ${token}`,
